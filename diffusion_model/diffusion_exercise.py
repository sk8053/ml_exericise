...
#B = np.repeat(A[None], 150, axis =0 )
#B = B.reshape(-1, 48, 50)[None]
...
if load_model == True:
    ...
    min_loss = 2e5
train_model = train_model (model = model, total_timesteps=total_timesteps, beta_max = beta_max, device = device)
...
for idx in range(0, total_timesteps, stepsize):
    ...
    image, noise = train_model.forward_data(image, t)
...
loss_avg = 0
loss_table = np.ones((total_timesteps,30))*10
...
for epoch in range(epochs):
    for step, (batch, cond) in enumerate(dataloader):
      ...
      loss, loss_weights = train_model.get_loss(model, batch, cond, t)
      
      #loss_weights = loss_weights.detach().cpu().numpy()
      #t_ = t.detach().cpu().numpy()
      #loss_table[t_, step%30] = loss_weights
    ...
    scheduler.step()
    if epoch % 2 == 0:
      print(f"Epoch {epoch} | step {step:03d} Loss: {loss_avg} ")
      train_model.sample_plot_image(dataset.shape[2:], cond[0][None])
    ...
    if epoch % 5 ==0: # compare distribution of pathloss per every 5 epoch
        ...
        pl_model_ = train_model.sample_dataset(n_img = 500, total_timestep = total_timesteps, 
                                               cond = cond_t,
